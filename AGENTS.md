...
For each first-use activity, check its **entry level and self-made tool/facility prerequisites** before recommending it (not just the later quest it supports). Treat quest XP already included in an actual-level checkpoint as spent once; do not add it again. At handoffs, audit consumable stocks *after* intervening uses—gross proceeds, banked items, and deposited kingdom funds are not liquid coins, and the same rune stack cannot fund two grinds. Introduce optional background resource loops with an explicit start, protected quest-item exception, and soft retirement point; never project their RNG output as a guarantee.
